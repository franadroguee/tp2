...
for fila in matriz:
    ...
    for pixel in fila:
        ...
        p_rojo.append(min(colores_disponibles, key=lambda x: abs(x - pixel[0])))
        p_verde.append(min(colores_disponibles, key=lambda x: abs(x - pixel[1])))
        p_azul.append(min(colores_disponibles, key=lambda x: abs(x - pixel[2])))
